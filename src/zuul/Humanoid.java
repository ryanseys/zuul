...
public abstract class Humanoid {
  ...
  /**
   * This is the constructor for the Humanoid. If no name is passed, the default
   * is the name of the class that instantiates it. The health is set, and the
  ...
  public Humanoid(int maxHealth, String name, Room room) {
    if (name == null) this.name = getClass().getName();
    else this.name = name;
    health = maxHealth;
  ...
  /**
   * Default constructor for the Humanoid. Calls the constructor above with the
   * default health and no name.
   */
  ...
  public boolean equals(Object o) {
    if (o instanceof Humanoid) return (health == ((Humanoid) o).health)
        && (inventory.equals(((Humanoid) o).inventory));
    return false;
  }
  ...
  public Item getBestItem() {
    ...
    for (Item i : inventory)
      if (i.isWeapon()) if (i.getValue() > tempItem.getValue()) tempItem = i;
    return tempItem;
  }
   ...
   * 
   * @return A string of the items if there are any, else a string saying there
   *         are no items.
   */
  public String getInventoryString() {
    String s;
    if (inventory.isEmpty()) s = "The " + this.getClass().getName()
        + " has no items";
    else {
