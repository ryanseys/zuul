...
public class Item implements Comparable<Item> {
  ...
  public int compareTo(Item i) {
    if (value > i.value) return 1; // this item is of greater value than the
                                   // parameter item
    else if (value == i.value) return 0; // the items have the same value (tie)
  ...
  public boolean equals(Object o) {
    if (o instanceof Item) return (name.equals(((Item) o).getName()));
    return false;
  }

  /**
   * Get the description of the item. Either add a or an to the string depending
   * on if it is a consonant or not.
   * 
  ...
  public String getDescription() {
    ...
    if (name.startsWith("A") || name.startsWith("E") || name.startsWith("I")
        || name.startsWith("O") || name.startsWith("U")) s += "an ";
    else s += "a ";
    s += name.toLowerCase() + ".\n";
    if (isWeapon()) s += "It can do " + value + " damage when used to fight.";
    else s += "It can recover " + value + " health when eaten.";
